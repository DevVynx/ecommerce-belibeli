...
import { CartEmpty } from "@/shared/components/store/CartEmpty";
import { useCartState } from "@/shared/states/cart";

export const CartDropdown = () => {
  const { items, count, updateQuantity, removeItem, rollback } = useCartState();
  ...
  const retailPrice = items.reduce((sum, item) => {
    return sum + item.product.variant.price * item.quantity;
  }, 0);
  ...
  const discount = retailPrice - subtotal;
            ...
            {items.length === 0 ? (
              ...
              <div className="flex flex-col">
                ...
                <div className="max-h-80 space-y-5 overflow-y-auto p-4">
                  {items.map((item) => {
                    const displayPrice = item.product.variant.isOnSale
                      ? item.product.variant.salePrice
                      : item.product.variant.price;
                    const percentDiscount = item.product.variant.isOnSale
                      ? Math.round(
                          100 - (item.product.variant.salePrice / item.product.variant.price) * 100
                        )
                      : 0;
                ...
                <div className="space-y-2 p-4 pt-3">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-semibold">R$ {retailPrice.toFixed(2)}</span>
                  </div>
                  {discount > 0 && (
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-muted-foreground font-semibold">Desconto:</span>
                      <span className="font-semibold text-red-500">-R$ {discount.toFixed(2)}</span>
                    </div>
                  )}
                  <Separator />
                  <div className="flex items-center justify-between text-sm">
